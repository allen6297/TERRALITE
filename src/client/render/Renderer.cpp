...
void renderMesh(const ChunkMesh& mesh, const TextureManager& textures) {
    ...
    for (const bool translucentPass : {false, true}) {
        ...
        for (const auto& surface : mesh.surfaces) {
            if (surface.translucent != translucentPass || !surface.vertexBuffer.isValid() || surface.vertexCount == 0) {
                continue;
            }
            ...
            glBindBuffer(GL_ARRAY_BUFFER, surface.vertexBuffer.openGlId());
...
void renderMeshWireframe(const ChunkMesh& mesh, const bool translucentOnly) {
    ...
    glEnableClientState(GL_VERTEX_ARRAY);
    for (const auto& surface : mesh.surfaces) {
        if ((translucentOnly && !surface.translucent) || !surface.vertexBuffer.isValid() || surface.vertexCount == 0) {
            continue;
        }
        glBindBuffer(GL_ARRAY_BUFFER, surface.vertexBuffer.openGlId());
